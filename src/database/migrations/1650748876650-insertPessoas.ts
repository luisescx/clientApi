...
export class insertPessoas1650748876650 implements MigrationInterface {
  name = "insertPessoas1650748876650";
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`funcao\` char(100) NULL DEFAULT 'NULL'`
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` DROP COLUMN \`data_admissao\``
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`data_admissao\` date NOT NULL`
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` DROP COLUMN \`data_nascimento\``
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`data_nascimento\` date NOT NULL`
    );
    await queryRunner.query(`ALTER TABLE \`pessoas\` DROP COLUMN \`cpf\``);
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`cpf\` char(100) NOT NULL`
    );
    await queryRunner.query(`ALTER TABLE \`pessoas\` DROP COLUMN \`rg\``);
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`rg\` char(100) NOT NULL`
    );
    await queryRunner.query(`ALTER TABLE \`pessoas\` DROP COLUMN \`nome\``);
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`nome\` char(100) NOT NULL`
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` DROP COLUMN \`id_pessoa\``
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD \`id_pessoa\` tinyint(255) NOT NULL AUTO_INCREMENT`
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` ADD PRIMARY KEY (\`id_pessoa\`)`
    );
    await queryRunner.query(
      `ALTER TABLE \`pessoas\` CHANGE \`id_pessoa\` \`id_pessoa\` tinyint(255) NOT NULL AUTO_INCREMENT`
    );
  }
}
